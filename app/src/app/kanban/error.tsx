...
export default function KanbanError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  const { message, details } = handleError(error, 'kanban');
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-black text-white">
      <div className="text-center space-y-4 max-w-md">
        <h1 className="text-4xl font-bold text-red-500">⚠️ Kanban Board Error</h1>
        <p className="text-zinc-400">The Kanban board encountered an error.</p>
