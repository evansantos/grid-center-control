const VALID_ACTIONS = new Set(['approve', 'reject', 'view', 'continue', 'pause', 'advance', 'batch']);
export function parseGridCallback(message) {
...
export function formatButtons(type, ...ids) {
    if (type === 'approval') {
 ...
 */
export function orchButtons(projectId, action, taskNums) {
    switch (action) {
        case 'launch':
            return [[
